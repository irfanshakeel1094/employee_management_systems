...
exports.createSalary = async (req, res) => {
  try {
    const { employee_id, base_salary, allowances, deductions, payment_date, month_year } = req.body;

    if (!employee_id || !base_salary || !month_year) {
      return res.status(400).json({
        success: false,
        message: 'Employee ID, base salary, and month year are required'
      });
    }
...
exports.updateSalary = async (req, res) => {
  try {
    const { id } = req.params;
    const { base_salary, allowances, deductions, payment_date } = req.body;
